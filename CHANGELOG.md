...
- i18n: user-facing output (scan list, confirm prompts, results) now supports English
  and Chinese. Language resolution: `--lang en|zh|auto` → `CODEX_CLEAN_LANG` env →
  `LANG`/`LC_ALL` → OS UI language → English.
- README: document `--lang` / `CODEX_CLEAN_LANG` usage.
- Roadmap: plan multi-agent support starting with `pi` (`@earendil-works/pi-coding-agent`).
