...
### 6. 追加修复：扫描列表数字与合计不一致（真机验证时发现）

在真实 `~/.codex` 上做端到端验证时发现：列表**各行列出的大小合计 129.5 MB，底部"预计可回收"却是 107.3 MB**，
差额 22.2 MB。定位结果：

| 类别 | 显示值(size_bytes) | 计入合计的值(reclaimable_bytes) |
|---|---|---|
| delete | 可删量 | 可删量 → 一致 |
| **vacuum** | **main + WAL + SHM（库总大小）** | **仅 WAL** → 不一致 |

例：logs-db 显示 34.0 MB（主库 20.60 + WAL 13.37 + SHM 0.03），但实际只贡献 13.37 MB。
这会让用户以为能回收 129.5 MB，与 v1.2.1「让数字真实」的目标相悖。

**修复**：列表列改为显示 `reclaimable_bytes`，当它与 `size_bytes` 不等时在后面标注库总大小，
如 `logs-db  13.4 MB  (库总 34.0 MB)`。列与页脚现可精确对齐。
**JSON 契约不变**：`size_bytes` 仍是总大小，`reclaimable_bytes` 仍是可回收量（两者语义不同，均保留），
稳定字段（`name`/`kind`/`path`/`planned_action`）未变。

**回归测试 14/14 通过**：列与页脚精确相等（153600 B = 153600 B）、中英文均正确标注、
JSON 契约未变、年龄过滤、精确释放 100KB、VACUUM 实测回收 2.0MB、`--age -1` 退出码 2。

### 7. 本地技能语言回归修复

上一步把仓库 `SKILL.md` 改为英文主版时，一并覆盖了你本机 `~/.codex/skills/codex-clean/SKILL.md`，
而你的 Codex 客户端是中文环境、原本那份一直是中文——属于我改动引入的回归。
已恢复：**仓库保持英文主版（对外一致），本地安装使用中文版**，两份都保留在本地目录。
